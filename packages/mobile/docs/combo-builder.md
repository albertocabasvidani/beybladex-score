...
**Aggiornamento a runtime (senza release)** — `REMOTE_PARTS_ENABLED` in `featureFlags.ts` (ON dove le combo sono attive; OFF in Production scoreboard-only → zero rete). Strategia A (stale-while-revalidate, apply-al-riavvio, **no hot-swap**): all'avvio `services/parts-remote.ts` → `hydratePartsFromCache()` legge la cache locale (`expo-file-system`, `documentDirectory/parts/parts.json`) e la attiva **solo se più fresca del bundle** (confronto lessicografico di `version` `YYYY-MM-DD`) via `setRegistry()`; poi `refreshPartsInBackground()` scarica `REMOTE_PARTS_URL` (= stesso master del combo finder di `sync-parts.js`), valida con `validatePartsRegistry` (shared, mirror runtime di `sync-parts.js`) e riscrive la cache — applicata al **prossimo** avvio (guardia min-interval 12h in AsyncStorage). `registry.ts` è ora sostituibile: `setRegistry()` azzera le 3 cache memoizzate, `getAllParts()` serve al diff. `partsStore` (transiente) tiene `status` (gate dei tab combo in `App.tsx`, evita il flash bundle→cache) e `newParts`; `NewPartsModal` annuncia "Aggiunte: X, Y, Z" (diff vs `seenPartIds` in AsyncStorage; il primo avvio non annuncia l'intero catalogo). Offline/errore/schema rotto → si resta sul bundle, nessun crash. Nota: le combo salvate mantengono lo snapshot delle stat, un aggiornamento dati non le ricalcola finché non si ri-seleziona la parte.
